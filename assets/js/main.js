...
resumeLink.href = '#'; // replace with your resume url if available
emailLink.href = 'mailto:you@example.com';
emailLink.textContent = 'you@example.com';
...
function buildHeaders(acceptPreview = false) {
  ...
  if (acceptPreview) headers['Accept'] = 'application/vnd.github.mercy-preview+json';
  return headers;
}
...
function renderGrid(list) {
  ...
  for (const r of list) {
    ...
    card.className = 'rounded-lg border p-4 bg-white dark:bg-slate-800 shadow-sm';
    card.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        <div>
          <h3 class="font-semibold text-lg"><a href="#" data-repo="${r.full_name}" class="project-link">${r.name}</a></h3>
          <p class="mt-1 text-sm text-slate-600 dark:text-slate-300 card-clamp">${r.description || (r._curated && r._curated.short) || '—'}</p>
          <div class="mt-3 flex flex-wrap gap-2 text-xs">${r._curated && r._curated.tags ? r._curated.tags.map(t => `<span class="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-700">${t}</span>`).join('') : (r.language ? `<span class="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-700">${r.language}</span>` : '')}</div>
        </div>
        <div class="text-right flex-shrink-0">
          <div class="text-sm text-slate-500 dark:text-slate-400">★ ${r.stargazers_count || 0}</div>
          <div class="mt-4 flex flex-col gap-2">
            <a class="inline-block text-xs px-2 py-1 rounded-md border" href="${r.homepage || r.html_url}" target="_blank">Open</a>
            <a class="inline-block text-xs px-2 py-1 rounded-md border" href="${r.html_url}" target="_blank">Repo</a>
          </div>
        </div>
      </div>
    `;
...
function buildLangOptions(list) {
  const langs = Array.from(new Set(list.map(r => r.language).filter(Boolean))).sort();
  langFilter.innerHTML = '<option value="">All languages</option>' + langs.map(l => `<option value="${l}">${l}</option>`).join('');
}
...
async function openProject(fullName) {
  ...
  const r = repos.find(x => x.full_name.toLowerCase() === fullName.toLowerCase());
  ...
  try {
    ...
    if (readme) {
      ...
      mdHtml = DOMPurify.sanitize(rawHtml, { ADD_TAGS: ["iframe"], ADD_ATTR: ["allow", "allowfullscreen", "frameborder", "scrolling"] });
    } else {
      ...
      <div class="mb-2">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <div class="mt-2 text-xs text-slate-400">Updated ${new Date(r.updated_at).toLocaleDateString()} • ★ ${r.stargazers_count}</div>
            <div class="mt-3 flex gap-2">${(curatedData && curatedData.tags ? curatedData.tags : []).map(t => `<span class="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-700">${t}</span>`).join('')}${topics.map(t => `<span class="px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-700">${t}</span>`).join('')}</div>
          </div>
          <div class="text-right">
            <a class="px-3 py-2 rounded-md border" href="${r.html_url}" target="_blank">Repository</a>
            ${r.homepage ? `<a class="ml-2 px-3 py-2 rounded-md border" href="${r.homepage}" target="_blank">Live</a>` : ''}
          </div>
        </div>
      </div>
  ...
  try {
    ...
    repos = fetched.map(r => {
      ...
      if (curated && curated[name]) {
        ...
        if (curated[name].description) r.description = curated[name].description;
      }
      return r;
    });
